...
class TakeItEasyGui:
    ...
    def __init__(self, game, net, radius=70):
        ...
        self.game = game
        self.state_size = 3*5*5+19*3*3+27  # 19*27
        self.root = Tk()
    ...
    def click3(self, e):
        for i, h in enumerate(self.board):
            if i in self.value_distributions and h.is_point_in_hexagon(e.x, e.y):
                ...
                plt.bar(self.net.support + self.game.compute_score() + r, d, width=3)
                plt.xlim((0, 307))
                plt.show()
                plt.pause(0.001)
                break
    ...
    def compute_expected_values(self):
        ...
        for i, a in enumerate(empty_tiles):
            rewards[i], _ = self.game.place(a)
            states[i] = self.game.encode()
            self.game.undo()
        with torch.no_grad():
            qd = self.net(states)
            q = rewards + (self.net.support * qd).sum(1)
