...
const std::int8_t spaces_on_lines[3][5][5] = {{{0, 1,  2,  INVALID_SPACE, INVALID_SPACE}, {3, 4, 5,  6,  INVALID_SPACE}, {7, 8, 9, 10, 11}, {12, 13, 14, 15, INVALID_SPACE}, {16, 17, 18, INVALID_SPACE, INVALID_SPACE}},
                                              {{0, 3,  7,  INVALID_SPACE, INVALID_SPACE}, {1, 4, 8,  12, INVALID_SPACE}, {2, 5, 9, 13, 16}, {6,  10, 14, 17, INVALID_SPACE}, {11, 15, 18, INVALID_SPACE, INVALID_SPACE}},
                                              {{7, 12, 16, INVALID_SPACE, INVALID_SPACE}, {3, 8, 13, 17, INVALID_SPACE}, {0, 4, 9, 14, 18}, {1,  5,  10, 15, INVALID_SPACE}, {2,  6,  11, INVALID_SPACE, INVALID_SPACE}}};

const std::int8_t lines_on_space[19][3] = {{0, 0, 2},
                                           {0, 1, 3},
                                           {0, 2, 4},
                                           {1, 0, 1},
                                           {1, 1, 2},
...
TakeItEasy::TakeItEasy(const std::optional<std::int32_t> seed) {
    ...
    board[INVALID_SPACE] = INVALID_PIECE;
    reset();
}
...
void TakeItEasy::reset() {
    for (std::int8_t i = 0; i < 19; ++i) {
        ...
        last_positions[i] = INVALID_SPACE;
...
std::int16_t TakeItEasy::computeScore() const {
    ...
    for (std::int8_t i = 0; i < 3; ++i) // direction i. e. |, \, or /
        for (std::int8_t j = 0; j < 5; ++j) { // row
            std::int8_t reference = numbers_on_pieces[board[spaces_on_lines[i][j][0]]][i];
            std::int8_t number_of_spaces = 5 - abs(2 - j);
            bool allEqual = board[spaces_on_lines[i][j][0]] != INVALID_PIECE;
            for (std::int8_t k = 0; k < number_of_spaces; ++k) // space
                // numbers_on_pieces[INVALID_PIECE][i] == -1
                allEqual &= (numbers_on_pieces[board[spaces_on_lines[i][j][k]]][i] == reference);
            score += allEqual * (number_of_spaces * reference);
        }
    return score;
}
...
std::int8_t TakeItEasy::computeScoreDelta(const std::int8_t pos) const {
    ...
    for (std::int8_t i = 0; i < 3; ++i) { // direction i. e. |, \, or /
        std::int8_t j = lines_on_space[pos][i]; // row
        std::int8_t reference = numbers_on_pieces[board[pos]][i];
        bool allEqual = board[pos] != INVALID_PIECE;
        for (std::int8_t k = 0; k < 5; ++k) // space
            allEqual &= (spaces_on_lines[i][j][k] == INVALID_SPACE)
                        | (numbers_on_pieces[board[spaces_on_lines[i][j][k]]][i] == reference);
        std::int8_t number_of_spaces = 5 - abs(2 - j);
        scoreChange += allEqual * (number_of_spaces * reference);
    }
    return scoreChange;
}
